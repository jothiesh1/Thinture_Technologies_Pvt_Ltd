...
fun getRotatedCarDrawable(context: Context, course: Float): BitmapDrawable {
    val angles = listOf(0, 22, 45, 67, 90, 112, 135, 157, 180, 202, 225, 247, 270, 292, 315, 337)
    val closest = angles.minByOrNull { abs(it - course.roundToInt()) } ?: 0
    val resourceName = "car_$closest"
        ...
        LaunchedEffect(playbackIndex, isSeeking) {
            if (!isSeeking && interpolatedPoints.isNotEmpty() && playbackIndex in interpolatedPoints.indices) {
                ...
                markerRef?.apply {
                    position = current
                    course?.let {
                        icon = getRotatedCarDrawable(context, it)
                    }
                ...
                update = { mv ->
                    ...
                    if (shouldLoadMap && !mapInitialized && interpolatedPoints.isNotEmpty()) {
                        ...
                        val m = Marker(mv).apply {
                            position = start
                            icon = getRotatedCarDrawable(context, 0f)
                            setAnchor(Marker.ANCHOR_CENTER, Marker.ANCHOR_CENTER)
                            isFlat = true
                        }
